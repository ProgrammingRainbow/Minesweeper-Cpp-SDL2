...
void Board::init() {
    ...
    this->reset(this->mine_count, true);
}

void Board::reset(int new_count, bool full_reset) {
...
void Board::setTheme(std::size_t new_theme) { this->theme = new_theme * 16; }

void Board::setSize(std::size_t new_rows, std::size_t new_columns) {
    this->rows = new_rows;
    ...
    for (std::size_t r = 0; r < this->front_vec.size(); r++) {
        ...
        for (std::size_t c = 0; c < this->front_vec[0].size(); c++) {
            ...
            SDL_RenderCopy(this->renderer.get(), this->image.get(),
                           &this->src_rects[rect_index + this->theme],
                           &dest_rect);
        }
    }
}
